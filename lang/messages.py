...
translations: Dict[str, Dict[str, str]] = {
    "zh_CN": {
        ...
        "template_content_placeholder": "输入提示词模板内容。使用{content}表示学习内容，{num_questions}表示问题数量。",
        ...
        "template_help_tips": "<h3>如何编写好的提示词模板？</h3><p>一个好的提示词模板应该：</p><ul><li>明确指定问题的类型和数量</li><li>详细说明难度和深度要求</li><li>说明是否需要解释或分析</li><li>指定是否需要源内容引用</li></ul><h3>可用变量</h3><ul><li>{content} - 将替换为学习内容</li><li>{num_questions} - 将替换为问题数量</li></ul><h3>示例</h3><pre>请根据以下内容设计 {num_questions} 个选择题：\n1. 每个问题应测试不同概念\n2. 选项应足够具有干扰性\n3. 每个问题应包含解释\n4. 避免简单的记忆类问题\n\n内容：\n{content}</pre>",
        "save_template": "保存模板",
    ...
    "en_US": {
        ...
        "template_example_description": "这是一个示例提示词模板，展示了如何设计一个好的提示词：",
        "template_help_description": "提示词模板编写指南",
        "template_help_tips": "<h3>How to Write Good Prompt Templates?</h3><p>A good prompt template should:</p><ul><li>Clearly specify the type and number of questions</li><li>Detail the difficulty and depth requirements</li><li>Indicate whether explanations or analysis are needed</li><li>Specify if source content references are required</li></ul><h3>Available Variables</h3><ul><li>{content} - Will be replaced with the learning content</li><li>{num_questions} - Will be replaced with the number of questions</li></ul><h3>Example</h3><pre>Please design {num_questions} multiple choice questions based on the following content:\n1. Each question should test different concepts\n2. Options should be sufficiently distracting\n3. Each question should include an explanation\n4. Avoid simple memorization questions\n\nContent:\n{content}</pre>",
        "save_template": "保存模板",
        "template_name_required": "请输入模板名称",
